...
public class TestCustomerFacade {
	...
	public static void main(String[] args) {
		...
		CouponDBDAO coupDBDAO = new CouponDBDAO();// for testing.
		...
		Coupon coup = new Coupon(1, "test1", new Date((long) (System.currentTimeMillis() * 0.9)),
				new Date((System.currentTimeMillis() + 10000)), 1, CouponType.FOOD, "nope", 200d, "no img");
		Coupon coup2 = new Coupon(2, "example2", new Date((long) (System.currentTimeMillis() * 1.25)),
				new Date((System.currentTimeMillis() * 2 + 20000)), 2, CouponType.HEALTH, "nope", 250d, "no img");
		Coupon coup3 = new Coupon(3, "test3", new Date((long) (System.currentTimeMillis() * 1.5)),
		...
		try {
			...
			System.out.println("Purchased all coupons(1-3)");
		} catch (CouponSystemException e) {
		...
		System.out.println("---------Check Get By- function.----------");
		System.out.println();
		try {
			System.out.println("Must get coup3");
			System.out.println(customer.getAllPurchasedCouponByType(CouponType.ELECTRICITY));
		} catch (CouponSystemException e) {
		...
		try {
			...
			System.out.println(customer.getAllPurchasedCouponByPrice(250d));
		} catch (CouponSystemException e) {
		...
		// } catch (CouponSystemException e) {
		// System.out.println("Faild to get unpurchased coupons");
		// }
